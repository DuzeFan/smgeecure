...
#'
#' @description Fit semiparametric accelerated failure time mixture cure model by the EM algorithm.
#' The details about the method can be found in Zhang and Peng (2007).
#' This function is indeed the same as that in Cai et al. (2012), but with different notations and sub-functions
#'
#' @aliases SMC.AFT.fit
#'
#' @param yobs right censored data which is the follow up time.
...
#' @param Z a vector or matrix of covariates corresponding to incidence part.
#' @param incidence specifies the link in incidence part. The "logit", "probit" or
#' complementary loglog ("cloglog") links are available. By default incidence = c("logit").
#' @param em.maxit specifies the maximum iteration number. If the convergence criterion
#' is not met, the EM iteration will be stopped after emmax iterations and the estimates
#' will be based on the last maximum likelihood iteration. The default \code{em.maxit = 100}.
#'
#' @importFrom survival Surv survreg
SMC.AFT.fit <- function(yobs,delta,X,Z,incidence=c("logit"),em.maxit=50){
  ...
  repeat{
    ...
    if(convergence.value >= 1e-7 & numit < em.maxit){
  ...
  convergence <- (numit<em.maxit & convergence.value < 1e-7)
...
#' (this convex objective function that will be minimized in each EM iteration)
#'
...
#' @title Survival function for latency
#'
#' @description Estimate the survival function of latency part used in the M-step of the EM algorithm for the AFT mixture cure model.
#' (this convex objective function that will be minimized in each EM iteration)
#'
#' @aliases SMC.AFT.St
#'
...
#' (this convex objective function that will be minimized in each EM iteration)
#'
#' @aliases SMC.AFT.eRes
#'
#' @param tobs right censored data which is the follow up time.
#' @param delta the censoring indicator, normally 0 = event of interest happens, and 0 = censoring.
#' @param X a vector or matrix of covariates corresponding to latency part.
#' @param bet unknown parameters corresponding to latency part.
#' @param gm the conditional expectation of the cure status given the observed data and the current.
#' estimates of the parameters in each E-step. The default \code{gm=rep(1,length(yobs))}.
...
#' @title Construct the correlation working matrix
#'
#' @description Construct the correlation working matrix
#'
#' @aliases CWM.corstr
#'
#' @param rho the common parameter needed to construct the correlation working matrix
#' @param ndim the dimension of the matrix that we will produce
...
#========================================================#
# Fit the semiparametric failure time mixture cure model ####
#========================================================#
...
#'
#' @export smgeecure
smgeecure <- function(formula, cureform, id, data, model = c("aft","ph"),
                      corstr = c("independence", "exchangeable","ar1"),  Var = TRUE,
                      nboot = 100, stdz = TRUE, esmax = 20, eps = 1e-04){
  ...
  if(model=="ph"){
    ...
    sdata <- cbind(id=id,data[,vars.name.response],X,Z[,!(vars.name.incidence %in% vars.name.latency)])
    formula  <- as.formula(paste("Surv(",paste(vars.name.response,collapse=","),")~",paste(vars.name.latency,collapse="+"),sep=""))
    cureform <- as.formula(paste("~",paste(vars.name.incidence,collapse="+"),sep=""))

    res <- geecure(formula=formula,cureform=cureform,data=sdata,id=sdata$id,
                   model="semi",corstr=corstr,Var=FALSE,boots=Var,nboot=nboot,esmax=esmax,eps=eps)
    # res <- geecure(formula=formula,cureform=cureform,data=sdata,id=sdata$id,
    #                model="semi",corstr=corstr,Var=Var,boots=FALSE,esmax=esmax,eps=eps)
    class(res) <- "list"
...
#' @title Semiparametric marignal accelerated failure time mixture cure model
#'
#' @description Fit semiparametric marignal accelerated failure time mixture cure model by the EM algorithm.
#' The details about the method can be found in Zhang and Peng (2007).
#' This function is indeed the same as that in Cai et al. (2012), but with different notations and sub-functions
#'
#' @aliases SMC.AFT.GEE
#'
#' @param tobs time to event of interest.
#' @param delta the censoring indicator, normally 0 = event of interest happens, and 0 = censoring.
#' @param id a vector which identifies the clusters. The length of \code{id} should be the same as the number of observations.
#' @param X a vector or matrix of covariates corresponding to latency part
#' @param Z a vector or matrix of covariates corresponding to incidence part
#' @param corstr a character string specifying the correlation structure. The following are permitted:
#' "independence"(\code{ind}), "exchangeable"(\code{ex}), "ar1"(\code{ar1}).
#' @param nboot specifies the number of bootstrap sampling. The default \code{nboot = 100}.
#' @param nwrong.rate the acceptable rate of wrong fitted bootstap samples.
#' @param Intercept Whether to include an intercept term in the latency part
#' @param control a list of iteration and algorithmic constants. ee \code{SMC.AFT.GEE.control} for their names and default values.
#' These can also be set as arguments to geese itself.
SMC.AFT.GEE <- function(tobs,delta,id,X,Z,corstr= c("ind","ar1","ex"),
                        Var=TRUE,nboot=100,nwrong.rate=0.4,Intercept=FALSE,
...
#==== The main function for fitting model ====#
#' @title Sub-fitting functions for semiparametric accelerated failure time mixture cure model
#'
#' @description Sub-functioins for fitting semiparametric accelerated failure time mixture cure model by the ES algorithm.
#'
#' @aliases SMC.AFT.GEE.fit
#'
#' @param tobs right censored data which is the follow up time.
#' @param delta the censoring indicator, normally 0 = event of interest happens, and 0 = censoring.
#' @param id a vector which identifies the clusters. The length of \code{id} should be the same as the number of observations.
...
#' @param control a list of iteration and algorithmic constants. See \code{SMC.AFT.GEE.control} for their names and default values.
SMC.AFT.GEE.fit <- function(tobs, delta, id, X, Z, corstr= c("ind","ar1","ex"),Intercept=FALSE,
  ...
  smcfit <- SMC.AFT.fit(tobs,delta,X,Z,em.maxit=control$emmax.smcure)
  if(Intercept==TRUE){
...
#' @title Auxiliary for Controlling AFT GEE Fitting
#'
#' @description Auxiliary function as user interface for \code{SMC.AFT.GEE} fitting.
#' Only used when calling 'SMC.AFT.GEE' or 'SMC.AFT.GEE.fit'.
#'
#' @aliases SMC.AFT.GEE.control
#'
#' @param emmax max number of iteration.
#' @param emeps relative error tolerance.
#' @param emtrace a binary variable, determine whether to display output for each iteration.
#' @param emmax.smcure max number of iteration.
#' @param maxiter.latency max number of iteration.
#' @param eps.latency relative error tolerance.
#' @param trace.latency a binary variable, determine whether to display output for each iteration.
#'
#' @seealso \code{\link{SMC.AFT.GEE}}
